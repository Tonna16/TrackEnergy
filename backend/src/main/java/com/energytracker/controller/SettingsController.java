...
public class SettingsController {
    ...
    public ResponseEntity<?> getSettings() {
        ...
        if (userId != null) {
            ...
            resp.put("currency", normalizeCurrency(s.getCurrency()));
            ...
            // Guest fallback
            Map<String,Object> resp = new HashMap<>();
            resp.put("electricityRatePerKWh", settingsService.getDefaultElectricityRate());
            resp.put("currency", settingsService.getDefaultCurrency());
    ...
    public ResponseEntity<?> updateSettings(@RequestBody Map<String, Object> incoming) {
        ...
        if (incoming.get("electricityRatePerKWh") instanceof Number value) {
            incomingRate = value.doubleValue();
        } else if (incoming.get("electricityRate") instanceof Number value) {
            incomingRate = value.doubleValue();
        }
        ...
        if (incomingRate == null || !Double.isFinite(incomingRate) || incomingRate < 0.0) {
            return ResponseEntity.badRequest().body(Map.of("message", "electricityRate must be a non-negative number."));
        }

        Object currencyValue = incoming.get("currency");
        if (currencyValue != null
            && !"USD".equalsIgnoreCase(currencyValue.toString())
            && !"EUR".equalsIgnoreCase(currencyValue.toString())) {
            return ResponseEntity.badRequest().body(Map.of("message", "currency must be USD or EUR."));
        }
        String incomingCurrency = currencyValue == null
        ...
        if (userId != null) {
            ...
            s.setCurrency(incomingCurrency);
            ...
            resp.put("currency", normalizeCurrency(saved.getCurrency()));
            User u = userService.getUserById(userId);
            resp.put("householdSize", u != null && u.getHouseholdSize() != null ? u.getHouseholdSize() : 2);
            return ResponseEntity.ok(resp);
        } else {
            ...
            resp.put("currency", incomingCurrency);
    ...
    private String normalizeCurrency(Object value) {
        return value != null && "EUR".equalsIgnoreCase(value.toString()) ? "EUR" : "USD";
    }
}
