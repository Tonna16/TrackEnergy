...
class EnergyUsageServiceWeeklyProjectionTest {
    ...
    void weeklyProjectionsExposeAlignedWeekStartAndWeekEnd() {
        ...
        assertEquals(projections.get(0).getTotalKwh() * 0.17, projections.get(0).getTotalCost(), 0.000001);
        assertEquals(7, projections.get(0).getDaysInPeriod());
        assertEquals("formula-estimate", projections.get(0).getSource());
    }
}
