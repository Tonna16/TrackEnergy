...
export default function UsageReportDownloads() {
  const { backendEnabled, demoMode } = useAppContext()
    ...
    return (
      <div className="card surface-glass">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold">Printable Energy Report</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Review recorded coverage or a clearly labeled Formula Estimate, then print or save as PDF locally.
            </p>
          </div>
          <Link
            to="/report"
            className="inline-flex items-center justify-center gap-2 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2 text-sm font-medium text-emerald-700 transition hover:bg-emerald-100 dark:border-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300"
          >
            <Printer size={16} />
  ...
  if (!backendEnabled || !getAuthToken()) return null
