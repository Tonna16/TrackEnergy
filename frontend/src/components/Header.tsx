...
export default function Header({ sidebarOpen, setSidebarOpen }: HeaderProps) {
  const { settings, updateSettings, backendEnabled } = useAppContext()
  const navigate = useNavigate()

  // determine login state
  const authToken = getAuthToken()
  const isLoggedIn = backendEnabled && Boolean(authToken)
  ...
  return (
    <header className="app-chrome surface-glass border-b border-gray-200/80 dark:border-gray-700/70">
        ...
        <div className="flex items-center space-x-4 relative">
          ...
          {!backendEnabled ? (
            <span className="rounded-full bg-sky-100 px-3 py-1 text-xs font-medium text-sky-700 dark:bg-sky-900/30 dark:text-sky-300">
              <span className="hidden sm:inline">Local Demo — data stays in your browser</span>
