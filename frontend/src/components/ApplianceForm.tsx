...
export default function ApplianceForm() {
  ...
  function validate(): boolean {
    ...
    if (isNaN(d) || !Number.isInteger(d) || d < 0 || d > 7) {
      errs.daysPerWeek = 'Days/Week must be a whole number from 0–7'
    }
    ...
    if (!isNaN(w) && w > 0 && !isNaN(h) && h > 0 && !isNaN(d) && d >= 0) {
      const dailyKwh = getKwhPerDay({
        ...
        estimatedDailyKWh: formData.estimatedDailyKWh === ''
          ? undefined
          : Number(formData.estimatedDailyKWh),
      })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // compute daily usage
      const newUsage = getKwhPerDay(base)
      // edit or add flow
      if (id) {
        const idNum = Number(id)
        if (isNaN(idNum)) {
        ...
        const oldUsage = getKwhPerDay(old)
        await updateAppliance({ id: idNum, ...base, isSample: old.isSample })
  ...
  return (
    <div className="max-w-3xl mx-auto pb-16 sm:pb-0">
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="flex items-center space-x-2">
            ...
            <label className="text-sm dark:text-gray-300">
              High-efficiency (informational only)
            </label>
          </div>
          <div className="flex items-center space-x-2">
            <input
        ...
        <div>
            <label className="block text-sm font-medium dark:text-gray-300">
              Manual daily kWh override for Formula Estimate (Optional)
            </label>
            <input
            ...
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              When set, this value replaces the wattage schedule in every Formula Estimate. Zero is a valid override.
            </p>
            {errors.estimatedDailyKWh && (
              <p className="mt-1 text-sm text-red-500">
                {errors.estimatedDailyKWh}
