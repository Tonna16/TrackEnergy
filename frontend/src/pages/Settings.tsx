...
export default function Settings() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!Number.isFinite(formData.electricityRate) || formData.electricityRate < 0) {
      setError('Enter a non-negative electricity rate in the selected currency.')
      setSaving(false)
      return
    }
    ...
    try {
      ...
      if (next.electricityRate !== old.electricityRate || next.currency !== old.currency) {
        await addNotification({
          type: 'info',
          title: 'Electricity Settings Updated',
          message: `Formula Estimates now use ${next.currency} ${next.electricityRate.toFixed(4)}/kWh.`,
        })
      }
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="card">
          ...
          <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">
            Enter your electricity rate in the selected currency. Changing USD/EUR preserves the
            numeric rate and only changes how costs are labeled; EnergyIQ does not perform exchange-rate conversion.
          </p>
        </div>
